            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_crop:
                        ...
                        crop_save_path = os.path.join(opt.crop_save_path, f"crop_{frame}_{int(cls)}.jpg")
...
# Define una función para redondear el color al color principal más cercano
                ...
                for i in range(len(names)):
                    if names[i] in clases_deseadas:
                        ...
                        # Formar la cadena de detección
                        if ids_color_names:
                            ids_str = ' - '.join(map(str, [icn[0] for icn in ids_color_names]))
                            colors_str = ' - '.join([f"({icn[1]})" for icn in ids_color_names])
                            detection_str = f"{len(ids_color_names)}{names[i]} (ID - {ids_str}) color {colors_str}"
                        else:
                            detection_str = f"0{names[i]}"
                        detections_with_ids_colors.append(detection_str)
